...
function Login() {
  ...
  const handleDemoLogin = () => {
    setEmail("demo@fitmind.ai");
    setPassword("password");
    setIsLoading(true);
    setTimeout(() => {
      login({ email: "demo@fitmind.ai", name: "Demo User" });
      setIsLoading(false);
      navigate("/dashboard");
    }, 1500);
  };
      ...
      >
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          ...
          <motion.button
            ...
            disabled={isLoading}
            className="w-full mt-3 bg-gray-800/80 hover:bg-gray-700 text-gray-300 py-3 rounded-xl font-semibold transition-colors flex items-center justify-center gap-2 border border-gray-700"
          >
            Quick Demo Login
          </motion.button>
        </form>
        ...
        <p className="text-center text-gray-400 mt-6">
          ...
          <Link to="/signup" className="text-green-400 hover:text-green-300 transition-colors font-medium ml-1">
            Create Account
          </Link>
        </p>
      </motion.div>
